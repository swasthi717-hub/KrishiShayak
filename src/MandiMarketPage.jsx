...
export default function MandiMarketPage() {
  ...
  const fetchPrices = async () => {
    try {
      ...
      console.error("Mandi page error:", err);
      setError("Unable to load mandi prices. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [normalizedRecords, selectedCrop]);
  ...
  const priceRows = useMemo(() => {
    ...
    normalizedRecords.forEach((item) => {
      ...
      if (
        !grouped[key].markets[item.market] ||
        item.modalPrice >
          grouped[key].markets[item.market].modalPrice
      ) {
        grouped[key].markets[item.market] = item;
      }
    });
  ...
  const getBestMarket = (row) => {
    ...
    markets.forEach((market) => {
      const record = row.markets[market];

      if (record && record.modalPrice > bestPrice) {
        bestPrice = record.modalPrice;
        bestMarket = market;
      }
    });
  ...
  const marketComparison = useMemo(() => {
    return markets
      .map((market) => {
        const recordsForMarket = selectedCropRecords.filter(
          (item) =>
            item.market.toLowerCase() ===
            market.toLowerCase()
        );

        if (!recordsForMarket.length) {
          return null;
        }

        const prices = recordsForMarket
  ...
  const bestMarket = useMemo(() => {
    if (!marketComparison.length) return null;

    return [...marketComparison].sort(
      (a, b) => b.price - a.price
    )[0];
  }, [marketComparison]);
  ...
  return (
    <Layout title="Mandi Market">
      <div className="space-y-6">
        ...
        {/* No data */}
        {!loading && !error && normalizedRecords.length === 0 && (
          <div className="rounded-3xl bg-white p-10 text-center shadow-sm ring-1 ring-[#e5dfd2]">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Try refreshing the data or check the API filters.
            </p>
          </div>
        )}
              ...
              <div className="flex flex-col gap-3 border-b border-[#e5dfd2] px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Modal prices from the latest mandi records
                  </p>
                </div>
              ...
              <div className="overflow-x-auto">
                ...
                <div className="min-w-[900px]">
                  ...
                  <div
                    ...
                    style={{
                      gridTemplateColumns: `1.7fr repeat(${markets.length}, 1.2fr) 1fr`,
                    }}
                  >
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            {item.variety || "Variety not specified"}
                          </p>
                        </div>
              ...
              <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-[#e5dfd2]">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Compare current {selectedCrop || "crop"} prices
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-500">
                        Prices shown here are based on the latest
                        mandi records returned by the government
                        data API. The comparison uses modal prices.
                      </p>
                    </div>
                  ...
                  {bestMarket ? (
                    <div className="mt-4">
                      ...
                      <p className="mt-3 text-xs leading-5 text-green-100">
                        This is a price comparison based on the
                        current API data. Actual selling decisions
                        should also consider transport, quality,
                    ...
                    <p className="mt-4 text-sm text-green-100">
                      ...
                      there is not enough current price data.
                    </p>
                  )}
